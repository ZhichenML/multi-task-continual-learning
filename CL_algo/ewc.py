...
class EWC_LOSS:
    ...
    def _update_fisher_params(self, dl, num_batch, num_labels):
            ...
            for i, batch in enumerate(dl):
                ...
                para = [v for v in self.model.parameters() if v.requires_grad==True]
                grad_log_liklihood1 = autograd.grad(log_likelihood_ner, para, allow_unused=True, retain_graph=True) #+ \
                grad_log_liklihood1 = [torch.tensor(0) if v == None else v for v in grad_log_liklihood1]
                self.model.zero_grad()
                grad_log_liklihood2 = autograd.grad(log_likelihood_cates, para, allow_unused=True, retain_graph=True)
