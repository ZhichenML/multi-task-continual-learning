...
from models.bert_for_ner_zcg import BertSpanForNer
...
from CL_algo.ewc import EWC_LOSS_span as EWC_LOSS
...
def train(args, train_dataset, valid_dataset, test_dataset, model, tokenizer, ewc):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        # input()
        result, cates_results = evaluate(args, model, tokenizer, valid_dataset, prefix=prefix, data_type='dev')
        # if global_step:
        #     result = {"{}_{}".format(global_step, k): v for k, v in result.items()}
        # results.update(result)
        # print('result here!!!!!!!!!!: ', result)
        
        args.writer.add_scalar('span_ner/eval/accuracy', result['acc'], epoch)
        args.writer.add_scalar('span_ner/eval/recall', result['recall'], epoch)
        args.writer.add_scalar('span_ner/eval/f1', result['f1'], epoch)
        ...
        if average_score  > best_score:
            ...
            test = False
            if test:
                test_result, cates_results = evaluate(args, model, tokenizer, test_dataset, prefix=prefix, data_type='test')
...
def evaluate(args, model, tokenizer, eval_features, prefix="", data_type='dev'):
    ...
    for step, f in enumerate(eval_features):
        ...
        subjects = f[9]
        cates_ids = torch.tensor([f[4]], dtype=torch.long).to(args.device)
        
        acts_ids = torch.tensor([f[5]], dtype=torch.long).to(args.device)
        model.eval()
        with torch.no_grad():
            inputs = {"input_ids": input_ids, "attention_mask": input_mask,
                      "start_positions": start_ids, "end_positions": end_ids, 'cates_ids': cates_ids,'acts_ids': acts_ids}
...
def process_data(train_set, task_name):
    ...
    train_dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_start_ids, all_end_ids, all_lens, all_cates_ids, all_acts_ids)
    ...
    if args.do_train:
        ...
        for ind, task_name in enumerate(train_dataset.task_setup):
            ...
            for past_ind in range(ind):
                ...
                test_tmp, cates_results = evaluate(args, model, tokenizer, test_set, prefix="", data_type='test')
                test_tmp['task_name'] = past_task_name
                past_results.append(test_tmp)
                past_results_cates.append(cates_results)
            test_result['task_name'] = task_name
            ...
            if ind % 2 == 0:
                np.savez(args.output_dir+ '/span_eval_matrix.npz', eval_matrix=eval_matrix, eval_matrix_cates = eval_matrix_cates)        
