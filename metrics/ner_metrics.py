...
class Classification_Score(object):
    def __init__(self):
        self.reset()
    def reset(self):
        self.origins = np.zeros((305,1))
        self.founds = np.zeros((305,1))
        self.rights = np.zeros((305,1))

    def update(self, labels, preds):
        for v in labels:
            self.origins[v] += 1
        for ind, v in enumerate(preds):
            self.founds[v] += 1
    ...
    def results(self):
        class_info = {}
        for ind,v  in enumerate(self.origins):
            origin = self.origins[ind]
            found = self.founds[ind]
            right = self.rights[ind]
            recall, precision, f1 = self.compute(origin, found, right)
            class_info[ind] = {"acc": np.round(precision, 4), "recall": np.round(recall, 4), "f1": np.round(f1, 4)}
        
        
        origin = np.sum(self.origins)
        found = np.sum(self.founds)
        right = np.sum(self.rights)
...
class SeqEntityScore(object):
    ...
    def update_chunks(self, label_chunks, pred_chunks):
            
        self.origins.extend(list(label_chunks))
        self.founds.extend(list(pred_chunks))
        self.rights.extend([pred_chunk for pred_chunk in list(pred_chunks) if pred_chunk in list(label_chunks)])
