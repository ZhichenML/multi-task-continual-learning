...
1. sh run_ner_crf.sh 运行
2. 相关参数设置在 tools/finetuning_argparse.py 和 run_ner_crf.sh
3. 代码一次性读入所有task的数据，再逐个对每个task进行训练
4. 为了方便小规模训练测试模型，在data.py line 799 设置每个task 样本量不超过10k，完整数据集训练耗时更长，注释799行代码即可
