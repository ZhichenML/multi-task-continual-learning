...
from models.bert_for_ner_zcg import BertSoftmaxForNer, BertCrfForNer, BertSpanForNer
...
def train(args, train_dataset, valid_dataset, test_dataset, model, tokenizer, ewc):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss += ewc._compute_consolidation_loss()
            ...
            #     # name_grad[name] = params.detach().numpy()
            #     # shape_list.append(params.size)
            #     # print("name: ", name, "require grad: ", params.requires_grad, "grad_value: ", params.grad)
            #     shape_list[name] = list(params.size())
            # print("len(shape_list): ",len(shape_list))
            # print("shpae list: ", shape_list)
            ...
            # # model.projector.update(hidden_states)
...
def evaluate(args, model, tokenizer, eval_dataset, prefix="", data_type='dev'):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        cates_pred_extend += [(cates_pred[i] == inputs["cates_ids"][i]).cpu() for i in range(len(cates_pred))]
        ...
        for ind, tmp in enumerate(logits.cpu().numpy()):
            ...
            # print("batch_onehot_label[ind]: ", batch_onehot_label[ind])
    ...
    # cates_accs = np.mean(np.numpy(cates_pred_extend))
    # print(cates_pred_extend)
    ...
    # logger.info("***** Cates results *****")
    # logger.info("acc:", cates_accs)
    logger.info
...
def main():
    ...
    if args.do_train:
        for ind, task_name in enumerate(train_dataset.task_setup):
            ...
            if ind % 2 == 0:
                np.savez(args.output_dir+ '/softmax_eval_matrix.npz', eval_matrix=eval_matrix, eval_matrix_cates=eval_matrix_cates)        
